...
class Brain:
    ...
    @classmethod
    def Init(cls, filename, data, net, criterion, optimizer, lr, betas=(0.9, 0.999), lr_decay=1, iterations=1,  batch_size=None, 
             print_every=1000, save=False, callback=None, dtype='float', device='cpu'):
        cls.brain = cls(filename, data, net, criterion, optimizer, lr, betas, lr_decay, iterations, batch_size, 
    ...
    def __init__(self, filename, data, net, criterion, optimizer, lr, betas, lr_decay, iterations, batch_size, 
                 print_every, save, callback, dtype, device):
    ...
    def run(self): 
        ...
        for i in range(self.iterations + 1):
            if self.batch_size is not None:
                ...
                regu = self.net.regularization(self.data.X_train[mask], self.data.y_train[:, mask])
            else:
                loss = self.__criterion(self.net(self.data.X_train), self.data.y_train)
                regu = self.net.regularization(self.data.X_train, self.data.y_train) 
            if i % self.print_every == 0 or i == self.iterations:
                loss_test = self.__test_criterion(self.net(self.data.X_test), self.data.y_test)
    ...
    def __init_criterion(self):
        if isinstance(self.net, DynNN):
            ...
            self.__test_criterion = self.net.criterion
